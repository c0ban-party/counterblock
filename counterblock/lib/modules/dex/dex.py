...
def get_pairs_with_orders(addresses=[], max_pairs=12):
    ...
    for my_pair in my_pairs:
        ...
        if my_pair['pair'] == 'RYO/XBP':  # XCP/BTC always in first
...
def get_pairs(quote_asset='XBP', exclude_pairs=[], max_pairs=12, from_time=None):
    ...
    for currency in quote_assets:
        if len(top_pairs) < max_pairs:
            ...
            for currency_pair in currency_pairs:
                ...
                if currency_pair['pair'] == 'XBP/RYO':  # XCP/BTC always in first
...
def get_assets_supply(assets=[]):
    ...
    if 'XBP' in assets:
        supplies['XBP'] = (util.call_jsonrpc_api("get_supply", {'asset': 'XBP'})['result'], True)
        assets.remove('XBP')
...
def get_markets_list(quote_asset=None, order_by=None):
    ...
    markets = []
    pairs = []
    currencies = ['XBP', 'XRYO'] if not quote_asset else [quote_asset]
    ...
    for pair in pairs:
        ...
        if market['base_asset'] == 'XBP' and market['quote_asset'] == 'RYO':
