    ...
    for asset in assets:
        ...
        if asset in [config.BTC, config.XCP]:
            if asset == config.BTC:
                ...
                supply = util.call_jsonrpc_api("get_supply", {'asset': 'XCB'}, abort_on_error=True)['result']
        ...
        assets_info.append({
            ...
            'listed': tracked_asset['listed'] if 'listed' in tracked_asset else None,
            'reassignable': tracked_asset['reassignable'] if 'reassignable' in tracked_asset else None,
    ...
    for i in range(len(raw)):  # oldest to newest
        if i == 0:
            ...
            history.append({
                ...
                'listed': raw[i]['listed'] if 'listed' in raw[i] else None,
                'reassignable': raw[i]['reassignable'] if 'reassignable' in raw[i] else None,
                'locked': raw[i]['locked'],
    ...
    if msg_data['locked']:  # lock asset
        ...
        if not tracked_asset:  # new issuance
            tracked_asset = {
                ...
                'listed': msg_data['listed'] if 'listed' in msg_data else None,
                'reassignable': msg_data['reassignable'] if 'reassignable' in msg_data else None,
                'locked': False,
